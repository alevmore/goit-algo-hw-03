...
def get_upcoming_birthdays(users=None):
    ...
    for user in users: # перебираємо користувачів
        ...
        bdate=dt.datetime.strptime(bdate, "%Y.%m.%d").date() # перетворюємо дату народження в об’єкт date
        ...
        if 0<=days_between<7: # якщо день народження протягом 7 днів від сьогодні
            if week_day<6: #  якщо пн-пт
                ...
                if dt.datetime.fromordinal(bdo+1).weekday()==0:# якщо неділя
                ...
                elif dt.datetime.fromordinal(bdo+2).weekday()==0: #якщо субота
